...
socket_path="/data/www/public_html/v3.adx.vn/server/socket"
script_path="/data/www/public_html/v3.adx.vn/server/scripts/production"
env="$1"
action="$2"
processJob()
{
...
start)
    ...
    echo '=========== Start worker subscribe-caching =============='
    sh $script_path/restart_job_subscribe.sh $env worker-subscribe-caching.php false
    echo '=========== Start worker monitor-buyer =============='
    sh $script_path/restart_job_socket.sh $env server-monitor.php false
