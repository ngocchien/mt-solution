...
$app = new Ratchet\App(SITE_URL_SOCKET, WEBSOCKET_PORT, '222.255.237.16', $loop);
//$app->route('/network', new Network($loop), array('*'));
$app->route('/click', new Click, array('*'));


$factory = new Factory($loop);
$channel = 'click_chart';
$factory->createClient('10.197.0.1:6504')->then(function (Client $client) use ($channel) {
    $client->subscribe($channel)->then(function () {
        echo 'Now subscribed to channel ' . PHP_EOL;
    });

    $client->on('message', function ($channel, $message) {
        call_user_func_array(array('EVENT\Click', 'click'), array($message));
    });
});
/*
