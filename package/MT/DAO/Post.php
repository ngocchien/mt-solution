...
class Post extends AbstractDAO
{
    ...
    public static function get($params){
        try {
            ...
            $adapter = self::getInstance('info_slave');
            $sql = new Sql($adapter);
    ...
    public static function buildWhere($params){
        ...
        if(!empty($params['source_id'])){
            $strWhere .= ' AND source_id = "'. $params['source_id'].'"';
        }

        if(!empty($params['my_id'])){
            $strWhere .= ' AND my_id = "'. $params['my_id'].'"';
        }
        ...
        if(!empty($params['post_title'])){
            $strWhere .= ' AND post_title = "'.$params['post_name'] . '"';
        }
        ...
        if(!empty($params['search'])){
            ...
            if(is_numeric($params['search'])){
                ...
                $strWhere .= ' AND (post_title like "%'.$params['search'].'%")';
            }
        }
