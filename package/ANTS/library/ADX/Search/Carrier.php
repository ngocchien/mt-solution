...
class Carrier extends SearchAbstract
{
    const INDEX_NAME = 'carriers';
    ...
    public function searchData($params = [])
    {
        try {
            ...
            $offset = $limit * ($page - 1);
            $sort = !empty($params['sort']) && is_array($params['sort']) ? $params['sort'] : ['carrier_id' => ['order' => 'desc']];
    ...
    public function __buildQuery($params)
    {
        ...
        if (!empty($params['in_carrier_id'])) {
            array_push($must, array(
                "terms" => array(
                    'carrier_id' => $params['in_carrier_id']
                )
            ));
        }
