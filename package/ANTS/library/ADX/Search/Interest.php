...
class Interest extends SearchAbstract
{
    const INDEX_NAME = 'interests';
    ...
    public function searchData($params = [])
    {
        try {
            ...
            $offset = $limit * ($page - 1);
            $sort = !empty($params['sort']) && is_array($params['sort']) ? $params['sort'] : ['interest_id' => ['order' => 'desc']];
    ...
    public function __buildQuery($params)
    {
        ...
        if (!empty($params['in_interest_id'])) {
            array_push($must, array(
                "terms" => array(
                    'interest_id' => $params['in_interest_id']
                )
            ));
        }
