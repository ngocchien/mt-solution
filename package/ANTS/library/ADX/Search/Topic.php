...
class Topic extends SearchAbstract
{
    ...
    public function searchData($params = [])
    {
        try {
            ...
            $offset = $limit * ($page - 1);
            $sort = !empty($params['sort']) && is_array($params['sort']) ? $params['sort'] : ['topic_id' => ['order' => 'desc']];
    ...
    public function __buildQuery($params)
    {
        ...
        if (!empty($params['in_topic_id'])) {
            array_push($must, array(
                "terms" => array(
                    'topic_id' => $params['in_topic_id']
                )
            ));
        }
