...
class Remarketing extends SearchAbstract
{
    const INDEX_NAME = 'remarketing';
    ...
    public function searchData($params = [])
    {
        try {
            ...
            $offset = $limit * ($page - 1);
            $sort = !empty($params['sort']) && is_array($params['sort']) ? $params['sort'] : ['remarketing_id' => ['order' => 'desc']];
    ...
    public function __buildQuery($params)
    {
        ...
        if (isset($params['remarketing_id'])) {
            array_push($must, array(
                "term" => array(
                    'remarketing_id' => $params['remarketing_id']
                )
            ));
        }


        if (!empty($params['in_remarketing_id'])) {
            array_push($must, array(
                "terms" => array(
                    'remarketing_id' => $params['in_remarketing_id']
                )
            ));
        }
