...
class Section extends SearchAbstract
{
    ...
    public function searchData($params = [])
    {
        try {
            ...
            $offset = $limit * ($page - 1);
            $sort = !empty($params['sort']) && is_array($params['sort']) ? $params['sort'] : ['section_id' => ['order' => 'desc']];
    ...
    public function __buildQuery($params)
    {
        ...
        if (!empty($params['in_section_id'])) {
            array_push($must, array(
                "terms" => array(
                    'section_id' => $params['in_section_id']
                )
            ));
        }
